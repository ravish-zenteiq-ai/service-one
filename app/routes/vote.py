...
async def do_vote(vote: Vote ,db: Session = Depends(get_db), user: int = Depends(get_current_user)):
    ...
    if (vote.dir):
        if vote_found:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Current user {user.id} has already voted to post")
        ...
        return{"message": status.HTTP_204_NO_CONTENT, "detail":"Post  Like deleted success"}
